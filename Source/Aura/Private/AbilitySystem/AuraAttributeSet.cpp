...
void UAuraAttributeSet::PostGameplayEffectExecute(const FGameplayEffectModCallbackData& Data)
{
	...
	if (Data.EvaluatedData.Attribute == GetIncomingDamageAttribute())
	{
		...
		if (LocalIncomingDamage > 0.f)
		{
			...
			const bool bBlock = UAuraAbilitySystemLibrary::IsBlockedHit(Props.SourceProperties->EffectContextHandle);
			const bool bCriticalHit = UAuraAbilitySystemLibrary::IsCriticalHit(Props.SourceProperties->EffectContextHandle);
	...
	if (Data.EvaluatedData.Attribute == GetIncomingXPAttribute())
	{
		...
		if (Props.SourceProperties->Character->Implements<UPlayerInterface>() && Props.SourceProperties->Character->Implements<UCombatInterface>())
		{
			...
			if (NumLevelUps > 0)
			{
				...
				IPlayerInterface::Execute_AddToPlayerLevel(Props.SourceProperties->Character, NumLevelUps);
...
void UAuraAttributeSet::ShowFloatingText(const FEffectPropertiesEnhanced& Props, float Damage, bool bBlockedHit, bool bCriticalHit) const
{
	if (Props.SourceProperties->Character != Props.TargetProperties->Character)
	{
		...
		{
			PC->ShowDamageNumber(Damage, Props.TargetProperties->Character, bBlockedHit, bCriticalHit);
			return;
		}
		if (AAuraPlayerController* PC = Cast<AAuraPlayerController>(Props.TargetProperties->Controller))
		{
			PC->ShowDamageNumber(Damage, Props.TargetProperties->Character, bBlockedHit, bCriticalHit);
		}
	}
...
void UAuraAttributeSet::SendXPEvent(const FEffectPropertiesEnhanced& Props)
{
	if (Props.TargetProperties->Character->Implements<UCombatInterface>())
	{
		...
		UAbilitySystemBlueprintLibrary::SendGameplayEventToActor(Props.SourceProperties->Character, GameplayTags.Attributes_Meta_IncomingXP, Payload);
	}
}

// Replication notifiers enable us to still read the old variable before replication
