...
- Enemy Ranged Attacks
  ...
  - For the montage, we do exactly like for the spear: in GA_RangedAttack BP, we update FacingTarget (for motion warping), we get one of the AttackMontages from our array (we created a BlueprintPure function for this so to avoid messy BP, and we applied it to GA_MeleeAttack too) and play it. We then wait for a gameplay event (tag sent by the AM_Event in the AnimationMontage) so to trigger the shot at the combat socket location at the right moment.
  - 