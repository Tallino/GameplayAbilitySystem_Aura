...
- Advanced Damage Techniques
  ...
  - So in AuraAbilitySystemBlueprintLibrary we create getters and setters for our 2 booleans taking them directly from the GE context. We call the setters in CalcDamage when we calculate if we get a block/crit, and we call the getters in PostGameplayEffect in AttributeSet in order to be able to manipulate the text based on the booleans. These booleans will be indeed passed through to the Damage Component class and then to the BP implementation of the SetText function. In BP, we create several branches with different text colours for each possible combination of block/critical (so 4 different colours)
  - We also want to show a message in one of these cases: 