...
11. Ability Tasks
    ...
    - We create a Widget for enemies health bar, using the Enemy class itself as a widget controller. From the OverlayWidgetController class we simply use the delegate that was created for Attribute change, and bind that delegate in BeginPlay, with a lambda associated that will broadcast the new value. The actual health bar change happens in newly created WBP_Progressbar, binding the return value of the broadcast to the FillPercent function.  
