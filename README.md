...
- Advanced Damage Techniques
  ...
  - Now we want to implement the concept of Damage types: we create a new subclass of AuraGameplayAbilities (AuraDamageGameplayAbilities) and make AuraProjectileSpell inherit from that. It will contain a map (DamageTypes) between Tags representing the damage types that the GA holds and the floats/damage values associated to that damage type (map that will be filled in the editor). Fire gameplay tag will be part of a similar array in the static gameplay tags singleton. Now this is where we will get the damage value now (so changing the code in ProjectileSpell and CalcDamage to now inject the new damage in SetByCallerMagnitude by looping in these new maps/arrays).
  - 