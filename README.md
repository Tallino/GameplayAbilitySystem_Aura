...
- Spell Menu
  ...
  - Now we want the globe to be become active and selectable when we level up (so, its status must be set to Eligible): we create a function on the ASC that can loop in the AbilityInfo and if we meet the level requirements for any of them, update their status. We first check if we already activated each ability during the loop with a new GetSpecFromAbilityTag function and if its nullpointer (no spec with tag exists in ActivatableAbilities struct), then we can actually continue (same for LevelUpRequirement check). If everything is fine, we create a spec, give it an Eligible status tag, and finally give it with GiveAbility.
  - Now we need a delegate that broadcasts when an ability status change, and our widget can subscribe to it in order to update the spell tree
