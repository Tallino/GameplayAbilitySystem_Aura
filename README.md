...
- Damage
  ...
  - Regarding death, we trigger it in the AttributeSet fatal damage check (as before), and the functions are declared (pure) in CombatInterface. Both Enemy and Character have quite the same implem, which is not an animation, rather it is the detachment of the weapon + ragdoll enabling.