...
- Enemy Finishing Touches
  ...
  - Our idea is to make the shaman spawn these minions: so we create a SummonGameplayAbility, that gets activated by the usual Attack tag in the ranged AI BT, and it immediately calls blueprint callable function GetSpawnLocations: idea of the function is to get some random spawn locations in a fan-shaped spread in front of the shaman, tuning through editor all interested parameters such as Min/Max SpawnLocation, NumOfMinions and SpawnSpread. The function simply rotates around the UpAxis of the Actor forward vector and then in a for loop it creates several vectors every i (step) starting from the actor and spreading out. Then we choose some random locations on these vectors.